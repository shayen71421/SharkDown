...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign In — SharkDown" },
      { name: "description", content: "Sign in with GitHub to edit repository files." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const authUrl =
    ...
    `/api/auth/callback?${new URLSearchParams({ redirect: window.location.origin + "/api/auth/callback" })}`;
    ...
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-background">
      ...
      <div className="mx-auto max-w-sm text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Sign in with GitHub to browse repositories, edit README files visually, and publish
        ...
        <p className="mt-8 text-xs text-muted-foreground">
          ...
          <span className="font-semibold text-foreground">read:user</span> scopes. We never store
